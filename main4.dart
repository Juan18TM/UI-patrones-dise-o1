...
class _AppBarExampleState extends State<AppBarExample> {
  ...
  void _changeColor(int newIndex, bool isAppBar) {
    setState(() {
      if (isAppBar) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<int>(
            tooltip: 'AppBar Colors',
            icon: const Icon(Icons.format_color_fill),
            onSelected: (index) => _changeColor(index, true),
            itemBuilder: (context) {
              return _appBarColors.asMap().entries.map((entry) {
                return PopupMenuItem<int>(
                  ...
                  child: ListTile(
                    ...
                    title: Text('AppBar Color ${entry.key + 1}'),
                  ),
                );
              }).toList();
            },
          ...
          PopupMenuButton<int>(
            tooltip: 'Scaffold Colors',
            icon: const Icon(Icons.format_paint),
            onSelected: (index) => _changeColor(index, false),
            itemBuilder: (context) {
              return _scaffoldColors.asMap().entries.map((entry) {
                return PopupMenuItem<int>(
                  ...
                  child: ListTile(
                    ...
                    title: Text('Scaffold Color ${entry.key + 1}'),
                  ),
                );
              }).toList();
            },
              ...
              ElevatedButton(
                onPressed: () {
                  setState(() {
                    ...
                    } else if (scrolledUnderElevation! >= 50.0) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Max elevation reached'),
                ...
                child: Text(
                    'subir sombra: ${scrolledUnderElevation ?? 'default'}'),
              ),
              ElevatedButton(
                onPressed: () {
                  setState(() {
                    if (scrolledUnderElevation == null) {
                      scrolledUnderElevation = 4.0;
                    } else if (scrolledUnderElevation! <= 0.0) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Min elevation reached'),
                          duration: Duration(seconds: 1),
                        ),
                      );
                    } else {
                ...
                child: Text('Bajar Sombra: ${scrolledUnderElevation ?? 'default'}'),
              ),
            ],
          ),
        ),
